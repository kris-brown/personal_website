...
***** Solution  
 ...
 - Problem with naive set theory when considering all sets which are not contained in themselves
...
***** <<X1.3.6>> :24:
 - Let's apply the /slice/ category [[E1.3.5]] to the category of integers under the relation $\leq$.
...
***** <<X1.3.10>> :25:
 - Consider the /fibered/ version of [[E1.3.9]]. Instead of fixing two objects, we fix two /morphisms/ (say $A \xrightarrow{\alpha} C, B \xrightarrow{\beta} C$) in a category to generate a new category $\mathcal{C}_{\alpha,\beta}$. 
...
***** Solution
 ...
 - However /f/ has no right inverse. We need ##a morphism $b\xrightarrow{f^{-1}}a$ such that $f \circ f^{-1} = id_b$, but no such morphism can exist.##
**** <<E1.4.4>>
***** Question
...
*** Part 3: Quotients
**** Notes 
 - Goal is to understand this assertion: "The quotient $A/_\sim$ is universal with respect to the property of mapping $A$ to a set in such a way that equivalent elements have the same image"
**** Examples 
***** <<X1.5.4>> 							 :33:
...
**** <<E1.5.10>> :todo:
***** Question
 - Define products and coproducts for /families/ (i.e. indexed sets) of objects of a category
 - Do these exist in $Set$?
 - (It is common to denote the product $A \times ... \times A$ by $A^n$)
***** Solution
 [ ] To do 
**** <<E1.5.11>> :todo:
***** Question
 ...
 - Use the universal property for quotients to establish that there are functions $(A \times B)/_\sim \rightarrow A/_\sim ,\ (A \times B)/_{\sim_A} \rightarrow B/_{\sim_B}$
 - Prove that $(A \times B)/_\sim$ with these two functions, satisfies the universal property for the product of $A/_{\sim_A}$ and $B/_{\sim_B}$
 - Conclude (without further work) that $(A \times B)/_\sim \cong A/_{\sim_A} \times B/_{\sim_B}$
***** Solution
 [ ] To do 
...
***** Solution
 - let  $h = \Pi_{g \in G}g$
 - Every element in $G$ has a unique inverse, $h = \Pi_{g \in G}g^{-1}$
 - $h^2=...g_2^{-1}g_1^{-1}eg_1g_2...=e$
 - Because /G/ is abelian we rearrange them into the above form. Because $h^2$ is /e/ (and $h\ne e$), we conclude /h=f/
**** <<E2.1.9>>
***** Question
...
***** Solution
 -  From proposition 1.14, we have $\exists k \in \mathbb{N}^+:  N = |gh|=k\cdot  lcm(m,n)$ 
...
***** Solution
 ...
 - The only possible definition of this homomorphism is $s_1s_2 \mapsto \phi_1(s_1)\cdot \phi_2(s_2)$. Note that $|G_1 \almalg G_2| \geq |G_1|+|G_2|$, but so although there are many set functions to /Z/ there is only one which satisfies the homomorphism property: $\phi(s_1s_2)\phi(s_1's_2')=\phi(s_1s_1's_2s_2')$
 - [ ] Not done yet?
**** <<E2.3.4>>
***** Question
...
****** Proof
 - For $a,b \in H$, we have $e \sim b$ and (left multiplying by $b^{-1}$) $b^{-1}\sim e$ and (left multiplying by /a/) $ab^{-1} \sim a \sim e$. 
 - By the subgroup criterion [[P2.6.2]] /H/ is a subgroup
 - Because /H/ is cloesd under multiplication, $a^{-1}bH \subseteq H$ which implies $bH \subseteq aH$ 
 - Our analysis was symmetric w/r/t /a/ and /b/, so $aH \subseteq bH$ and therefore $aH=bH$
...
**** <<E2.10.3>> :todo:
***** Question
 - Let $(G,\cdot)$ be a group and suppose $G \times G \xrightrarrow{\circ} G$ is a group homomorphism (w/r/t $\cdot$) such that $(G, \circ)$ is /also/ a group. 
...
**** Examples
***** <<X3.1.3>> :121:
 - The ##trivial ring / zero ring## is a ring {a} with a+a=a and $a \cdot a = a$ . Note that 0=1 in this ring.**** Propositions
**** Definitions
***** <<D3.1.8>> :122:
****** Term
